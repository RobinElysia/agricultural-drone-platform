...
export const authenticate = (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const token = AuthUtil.extractTokenFromHeader(authHeader as string)
...
export const errorHandler = (err: Error, req: Request, res: Response, next: NextFunction) => {
  ...
  if (bodyParserError.type === 'entity.too.large' || bodyParserError.status === 413 || bodyParserError.statusCode === 413) {
    return res.status(413).json(
      ResponseUtil.badRequest('Request body too large. Reduce image size or increase BODY_LIMIT.')
    )
  }
