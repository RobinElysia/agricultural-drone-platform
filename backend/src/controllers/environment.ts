...
// 风力等级映射
const windPowerToLevel = (windPower: string): number => {
  if (windPower.includes('≤3')) return 2
  if (windPower.includes('4-5')) return 4
  if (windPower.includes('6-7')) return 6
  if (windPower.includes('8-9')) return 8
  if (windPower.includes('≥10')) return 10
  return 2
}
...
export class EnvironmentController {
  ...
  // 订阅环境更新（模拟）
  static async subscribeEnvironment(req: Request, res: Response) {
    try {
      // 模拟订阅成功
  ...
  // 获取环境历史
  static async getEnvironmentHistory(req: Request, res: Response) {
    try {
      // 从Redis获取环境历史
